...
class TripletImageLoader(torch.utils.data.Dataset):
    def __init__(
        ...
        with open(os.path.join(rootdir, "%s.json" % split), "r") as data_json:
    ...
    def sample_negative(self, outfit_id, item_id, item_type):
        ...
        while item_out == item_id and attempts < 100:
            choice = candidate_sets[np.random.choice(len(candidate_sets))]
            items = self.category2ims[item_type][choice]
            item_index = np.random.choice(len(items))
            ...
            for i in range(n_items - 1):
                ...
                for j in range(i + 1, n_items):
                    ...
                    if metric is None:
                        ...
                        outfit_score += metric(embed1 * embed2)
